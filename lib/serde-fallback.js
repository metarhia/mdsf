...
class Parser {
  ...
  isWhitespace(character) {
    return (
      ...
      ['\u{0020}', '\u{00A0}', '\u{1680}', '/[\u{2000}-\u{200A}/u'].includes(
        character
      )
    );
  }
  ...
  parseValue() {
    ...
    if (this.isLetter(look)) {
      ...
      this.isInitialDigit(look) ||
      this.string.includes('NaN') ||
      this.string.includes('Infinity')
    ) {
  ...
  parseNumber() {
    ...
    if (this.string.includes('NaN')) {
      while (this.isInitialIdentifierCharacter(this.lookahead())) {
        this.advance();
      }
      return NaN;
    }
    ...
    if (isNaN(value)) this.throwError('Invalid format');
  ...
  parseDecimal() {
    ...
    if (this.string.includes('Infinity')) {
      while (this.isInitialIdentifierCharacter(this.lookahead())) {
        number += this.advance();
      }
      if (!isFinite(number)) {
  ...
  parseMachineInteger(base) {
    ...
    let number = '';
    if (!checkDigit(this.lookahead())) {
      throw new SyntaxError('Empty number value');
    }
  ...
  checkIdentifier() {
    while (this.lookahead() && this.lookahead() !== ':') {
      if (this.lookahead().charCodeAt(0) >= 192) {
        return true;
      }
      this.advance();
    }
