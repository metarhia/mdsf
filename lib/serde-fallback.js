...
class Parser {
  ...
  // Check if a given character is a letter character
  //   character - a character to check
  //
  isLetter(character) {
    return (
  ...
  skipClutter() {
    while (true) {
      ...
      if (this.isWhitespace(look) || this.isNewline(look)) {
  ...
  parseObjectKey() {
    ...
    if (this.isDecimalDigit(this.lookahead()) || this.lookahead() === '.') {
      return this.parseNumber();
    }
