...
//
jsrs.parse = function parse(data) {
  if (Buffer.isBuffer(data)) {
    data = data.toString();
  }

  var parser = new JsrsParser(data);
  return parser.parse();
};
...
//
jsrs.parseNetworkPackets = function parseNetworkPackets(data, packets) {
  var chunks = data.split('\u0000');
  var readyPacketsCount = chunks.length - 1;

  for (var i = 0; i < readyPacketsCount; i++) {
    var parser = new JsrsParser(chunks[i]);
    var packet = parser.parseObject();
